...
1. Explain the difference between `Relational Databases` and `SQL`.
    -Relational databases are databases that store related data in a tabular format much like spreadsheets. SQL is structured query language and standard language much like english for storing, manipulating and retrieving in the database.
2. Why do tables need a `primary key`?
    -A primary key is a way to identify each entry in the database uniquely.
3. What is the name given to a table column that references the primary key on another table.
    - A foreign key.
4. What do we need in order to have a _many to many_ relationship between two tables.
    -We need an intermediary or junction table that holds foreign keys that reference the primary key on the related tables.
